    ...
    <fieldset class="examresult">
        ...
        <table>
            ...
            @foreach($questions as $question)
                    <tr>
                        @if(!key_exists($count, $anses))<?php $count++ ?>
                    <p>{{ $question['question'] }}<img src="{{ asset('img/red-x.jpg') }}" height="15" width="15"></p>@continue@endif<p>{{ $question['question'] }}</p><p style="display: inline;">{{ $question['ans1'] }}<?php if (in_array('ans1', $question['correct'])) {if (in_array('ans1', $anses[$count])) { ?></p> <img src="{{ asset('img/kvacica.jpg') }}" height="12" width="12"><?php } }elseif (in_array('ans1', $anses[$count])) { ?><img src="{{ asset('img/red-x.jpg') }}" height="15" width="15"> <?php }  ?> <br>

                    <p style="display: inline;">{{ $question['ans2'] }}
                        <?php if (in_array('ans2', $question['correct'])) {
                            if (in_array('ans2', $anses[$count])) {
                                ?></p> <img src="{{ asset('img/kvacica.jpg') }}" height="12" width="12"> <?php
                            }
                        } elseif (in_array('ans2', $anses[$count])) { ?>
                        <img src="{{ asset('img/red-x.jpg') }}" height="15" width="15"> <?php
                        }  ?> <br>
